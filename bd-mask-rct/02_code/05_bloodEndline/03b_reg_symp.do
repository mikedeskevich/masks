...
*           symptomatic status at endline as an outcome.
* ---------------------------------------------------------------------
...
regsave using "${table_blood_end}\main\5a_symp_pool.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'

* No Baseline Controls
reghdfe symp treatment, absorb(pairID) vce(cluster union)
regsave using "${table_blood_end}\main\5b_symp_pool_no_base.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'
...
regsave using "${table_blood_end}\main\6a_symp_mask.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'

* No Baseline Controls
reghdfe symp treat_surg treat_cloth, absorb(pairID) vce(cluster union)
regsave using "${table_blood_end}\main\6b_symp_mask_no_base.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'
...
clear
svmat double Symp
save "${table_blood_end}\main\7a_symp_pool.dta", replace

* Pooled, Without Baseline Controls
clear
svmat double SympNoBase
save "${table_blood_end}\main\7b_symp_pool_no_base.dta", replace

* Mask Type
clear
svmat double SympMask
save "${table_blood_end}\main\8a_symp_mask.dta", replace

* Mask Type, Without Baseline Controls
clear
svmat double SympMaskNoBase
save "${table_blood_end}\main\8b_symp_mask_no_base.dta", replace
...
format mean* %5.4f
save "${table_blood_end}\main\summ_stats_symp.dta", replace
