...
mat Symp = J(4,6,.)
mat SympNoBase = J(4,6,.)
mat SympMask = J(4,10,.)
mat SympMaskNoBase = J(4,10,.)
mat summStats = J(4,3,.)
...
regsave using "${table_blood_end}\simulation\age_bin\5a_symp_pool.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'

* No Baseline Controls
reghdfe symp treatment, absorb(pairID) vce(cluster union)
regsave using "${table_blood_end}\simulation\age_bin\5b_symp_pool_no_base.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'
...
regsave using "${table_blood_end}\simulation\age_bin\6a_symp_mask.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'

* No Baseline Controls
reghdfe symp treat_surg treat_cloth, absorb(pairID) vce(cluster union)
regsave using "${table_blood_end}\simulation\age_bin\6b_symp_mask_no_base.dta", table(symp_`doc_title', format(%5.4f) parentheses(stderr) asterisk()) addlabel(subsample, `subsample') `replace_choice'
...
clear
svmat double Symp
save "${table_blood_end}\simulation\age_bin\7a_symp_pool.dta", replace

* Pooled, Without Baseline Controls
clear
svmat double SympNoBase
save "${table_blood_end}\simulation\age_bin\7b_symp_pool_no_base.dta", replace

* Mask Type
clear
svmat double SympMask
save "${table_blood_end}\simulation\age_bin\8a_symp_mask.dta", replace

* Mask Type, Without Baseline Controls
clear
svmat double SympMaskNoBase
save "${table_blood_end}\simulation\age_bin\8b_symp_mask_no_base.dta", replace
...
format mean* %5.4f
save "${table_blood_end}\simulation\age_bin\summ_stats_symp.dta", replace
