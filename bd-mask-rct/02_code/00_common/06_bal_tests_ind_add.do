* ---------------------------------------------------------------------
* Title:    Additional Balance Tests (Individual-Level)
...
regsave using "${table_com}\3_balance_test_ind_add.dta", table(hh_count, format(%6.0f) parentheses(stderr) asterisk()) addlabel(subsample, "Household Count") replace

* (2) Sex
reghdfe sex treatment, absorb(pairID) vce(cluster union)
regsave using "${table_com}\3_balance_test_ind_add.dta", table(sex, format(%6.4f) parentheses(stderr) asterisk()) addlabel(subsample, "Proportion Female") append

* (3) Age
reghdfe prop_below_30 treatment, absorb(pairID) vce(cluster union)
regsave using "${table_com}\3_balance_test_ind_add.dta", table(below_40, format(%6.4f) parentheses(stderr) asterisk()) addlabel(subsample, "Proportion Below 30") append

* (4) Household Size
reghdfe hh_size treatment, absorb(pairID) vce(cluster union)
regsave using "${table_com}\3_balance_test_ind_add.dta", table(hh_size, format(%6.4f) parentheses(stderr) asterisk()) addlabel(subsample, "No. Adults per HH") append
...
* Age
qui reg prop_below_30 treatment i.pairID
estimates store r3
